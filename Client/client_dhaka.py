...
sio = socketio.Client()
sio.connect('http://communication.dhk.com:8080')
...
def send_rating(r_name, d_name):
    ...
    requests.post("http://dhaka.server.com:80/rating", json=rate_req)
...
while True:
    ...
    requests.post("http://dhaka.server.com:80/api/rider", json=rider_req)
    # print('Sending Driver info to Ride_share\n')
    requests.post("http://dhaka.server.com:80/api/driver", json=driver_req)
